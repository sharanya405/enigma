class Rotor:
    ...
    def setChar(self, cposn):
        self._setting = self._permutation.alphabet().toInt(cposn)
    ...
    def atNotch(self):
        for notch in self._notches:
            if self.alphabet().toInt(self._notches[notch]) == self._setting:
                return True
        return False

    # Advance me one position, if possible.
    def advance(self):
        set(self._permutation.wrap(self._setting + 1))